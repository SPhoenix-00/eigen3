...
# Brax/Gymnasium are imported lazily so `from evorl.envs import Env` does not load
# Brax → MuJoCo MJX → optional NVIDIA Warp (which prints noisy messages when absent).


def __getattr__(name: str):
    if name == "create_brax_env":
        from .brax import create_brax_env as _fn

        return _fn
    if name == "create_wrapped_brax_env":
        from .brax import create_wrapped_brax_env as _fn

        return _fn
    if name == "create_gymnasium_env":
        from .gymnasium import create_gymnasium_env as _fn
